...
$(greg).
 bind("end", function(event, name){
  ...
  if(greg.promocpt < greg.promo.length){
        $.ajax({
                url: "../../wp-content/themes/celsa-misc/data/" + greg.promo[greg.promocpt].name,
                cache: false,
                dataType: "json",
...
$.ajax({
    url: "../../wp-content/themes/celsa-misc/data/list.json",
    cache: false,
    dataType: "json",
...
function printstudents(data){
    ...
    function(i, item){
      ...
      if(item.blog != ""){
        ...
        "<div class='photo'><img  class='photo' src='../../wp-content/themes/celsa-misc/images/photos/" + item.image + "' title='" + item.name + "'/></div>" +
        ...
        "</div>" +
        "<div class='meta_student'><h3>Recherche universitaire : </h3>" + item.resume_title + "<br/>" +
        "<h3>Centres d'int&eacute;r&ecirc;ts : </h3><p id='interest_" + slugstudent + "'></p></div>"
        ...
        $.each(item.social,
        function(j, link){
            $("<a target='_blank' href='" + link.url + "'>").html("<img class='blog' src='../wp-content/themes/celsa-misc/images/icons/" +greg.iconset+ "/"+ link.name.toLowerCase() + ".png' title='" + link.name + "'/>"
...
function printresult(data){
    ...
    function(i, item){
      ...
      if(item.blog != ""){
        $("<li class='student'>").html(
        "<div class='photo'><img  class='photo' src='../wp-content/themes/celsa-misc/images/photos/" + item.image + "' title='" + item.name + "'/><div class='promo_legend'>"+item.promo+"</div></div>" +
        "<div class='infosStudent'><h3>" + item.firstname + " <span style='text-transform: uppercase;'>" + item.name + "</span></h3>" +
        "<a target='_blank' href='" + item.blog + "' class='blog'>Blog</a>" +
        "<div class='block_blog' id=social_" + slugstudent + "></div>" +
        "</div>" +
        "<div class='meta_student'><h3>Recherche universitaire : </h3>" + item.resume_title + "<br/><br/>" +
        "<h3>Centres d'int&eacute;r&ecirc;ts : </h3><p id='interest_" + slugstudent + "'></p></div>"
        ).appendTo("#title_" + slugpromo);
        }
        else{
